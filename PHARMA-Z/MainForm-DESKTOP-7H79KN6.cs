...
namespace PHARMA_Z
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            conn.Open();
            query = "SELECT Theme_Toggle FROM System_Configuration";
            SqlCommand cmd = new SqlCommand(query, conn);
            SqlDataReader sdr = cmd.ExecuteReader();
            while (sdr.Read())
            {
                state = (bool)sdr["Theme_Toggle"];
            }
                if(state == true){
            ...
            {
                if (Search_tb.Text == "" || Search_tb.Text == "Search For Brand")
                {
                    Search_tb.Text = "Search For Generic Drug";
                }
            }
            else if (Brand.Checked == true)
            {
                if (Search_tb.Text == "" || Search_tb.Text == "Search For Generic Drug")
                {
        ...
        private void darkThemeOn()
            {
                SqlConnection conn = new SqlConnection(conString);
                conn.Open();
                query = "UPDATE System_Configuration SET Theme_Toggle = 1";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.ExecuteNonQuery();
                conn.Close();
        ...
        private void darkThemeOff()
            {
                SqlConnection conn = new SqlConnection(conString);
                conn.Open();
                query = "UPDATE System_Configuration SET Theme_Toggle = 0";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.ExecuteNonQuery();
                conn.Close();
        ...
        private async void Send_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                while (sdr.Read())
                {
                    from = sdr["EmailFrom"].ToString();
                    pwd = sdr["Pwd"].ToString();
                    to = sdr["EmailTo"].ToString();
                }
        ...
        private void Search_tb_Click(object sender, EventArgs e)
            {
                if (Search_tb.Text == "Search For Generic Drug" || Search_tb.Text == "Search For Brand")
                {
                    Search_tb.Text = "";
                }
            }
